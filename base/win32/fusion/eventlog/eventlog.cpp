...
/*
	RtlCopyUnicodeString alternative
*/
void CopyUnicodeString(UNICODE_STRING* dest, const UNICODE_STRING* src) {
    if (dest && src) {
...
FusionpOpenEventLog()
{
    ...
    if (hEventLog == NULL)
    {
        ::FusionpDbgPrintEx(FUSION_DBG_LEVEL_ERROR, "SXS.DLL: FusionpOpenEventLog/RegisterEventSourceW failed %lx\n", static_cast<ULONG>(GetLastError()));
        goto Exit;
    }
    ...
    )
{
	...
	UNICODE_STRING rgpsBuff;
	
	CopyUnicodeString(&rgpsBuff, *rgps);
	LPCWSTR rgpsL = static_cast<LPCWSTR>(rgpsBuff.Buffer);
